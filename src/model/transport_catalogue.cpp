...
const BusInfo TransportCatalogue::GetBusInfo(const Bus* current_bus) const
{
    ...
    bus_info.type = current_bus->type;
    bus_info.capacity = current_bus->capacity;
    bus_info.operating_time = current_bus->operating_time;
...
double TransportCatalogue::ComputeTfIdfForBus(const Bus* bus,
                                               ...
                                               const BusProperties::Type bus_type,
    ...
    if (stopname.has_value() && !stopname->empty())
    {
        ++total_terms;
        for (const auto& stop : bus->stops)
        {
            if (stop->name == stopname->toString())
            {
                ++count_term_in_bus;
                break;
            }
    ...
    if (operating_time != BusProperties::OperatingTime::undefined)
    {
        ++total_terms;
        if (bus->operating_time == operating_time)
        {
            ++count_term_in_bus;
        }
    }
    if (direction != BusProperties::Direction::undefined)
    {
        ++total_terms;
    ...
    if (bus_type != BusProperties::Type::undefined)
    {
        ++total_terms;
